                                     ...
                                     unsigned short ushCodeLength, int iSrcFD)
{
    ...
    {
        TRACESVR(LOG_LEVEL_DETAIL, "In CCodeDispatcher::DispatchOneCode GetExternalSocketByFD(%d) is nullptr\n", iSrcFD);
        return -1;
    }
                                       ...
                                       unsigned short ushCodeLength, int iSrcFD)
{
    ...
    TExternalClientSocket *pstExternalClientSocket = m_pstFDPool->GetExternalSocketByFD(iSrcFD);
    if (nullptr == pstExternalClientSocket)
    {
        //TRACESVR(LOG_LEVEL_DETAIL, "In CCodeDispatcher::DispatchToDefault DispatchToDstServer nullptr == pstExternalClientSocket \n");
        return -1;
    }
                                         ...
                                         unsigned short ushCodeLength, int iSrcFD, unsigned int uiRealUin)
{
    ...
    if(!pstSocket)
    {
        //TRACESVR(LOG_LEVEL_DETAIL, "In CCodeDispatcher::DispatchToDstServer pstSocket is nullptr\n");
        return -1;
    }
...
int CCodeDispatcher::SendToInternalServer(TInternalServerSocket* pstSocket, int iCodeLength, const char* pszCodeBuffer)
{
    if(!pstSocket)
    {
        //TRACESVR(LOG_LEVEL_DETAIL, "In CCodeDispatcher::SendToInternalServer pstSocket:nullptr\n");
        return -1;
    }
...
int CCodeDispatcher::SendRemain(TInternalServerSocket* pstSocket)
{
    if(!pstSocket)
    {
        //TRACESVR(LOG_LEVEL_DETAIL, "In CCodeDispatcher::SendRemain pstSocket:nullptr\n");
        return -1;
    }
...
int CCodeDispatcher::PackNetHead_V2(char* pNetHeadBuffer, int iSrcFD, unsigned int uiRealUin)
{
    ...
    pstNetHead->m_uiCodeTime = htonl(time(nullptr));
