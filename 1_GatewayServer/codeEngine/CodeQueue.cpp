...
{
    if(pszKeyFileName == nullptr || uiMaxQueueLength <= 0)
    {
        return nullptr;
    }
    ...
    pstCodeQueue->m_aucQueue = nullptr;
    //计算CodeQueue偏移
...
{
    if(pucInCode == nullptr || iCodeLength <= 0)
    {
    ...
    unsigned char* pucQueueBuf = nullptr;
    GetQueueBuf(pucQueueBuf);

    if(pucQueueBuf == nullptr)
    {
        SetErrorNO(EEN_CODEQUEUE__NULL_POINTER);
...
{
    if(pucOutCode == nullptr || iMaxOutCodeLen <= 0)
    {
        SetErrorNO(EEN_CODEQUEUE__NULL_POINTER);
